...
export class BackendLiveService {
  ...
  public static async iniciarSessao(
    ...
    void LocalWheelRecognizer.warmup();
  ...
  public static async processarFrame(
    ...
    if (framePayload.metadata?.resultadoScreenDetected !== undefined) {
      ...
      const fallbackDetector = WheelResultScreenDetector.detectResultScreen({
        width: largura,
        height: altura,
        base64Data: cleanBase64,
        isBlackOrEmpty: !conteudoVisual,
      ...
      resScreenDetection = {
        ...
        roi: {
          ...fallbackDetector.roi,
          ...metaRoi,
          symbolCropX: metaRoi.symbolCropX ?? fallbackDetector.roi?.symbolCropX ?? 0,
          symbolCropY: metaRoi.symbolCropY ?? fallbackDetector.roi?.symbolCropY ?? 0,
          symbolCropWidth: metaRoi.symbolCropWidth || fallbackDetector.roi?.symbolCropWidth || 153,
          symbolCropHeight: metaRoi.symbolCropHeight || fallbackDetector.roi?.symbolCropHeight || 153,
    ...
    if (!roi.found && !resScreenDetection.resultadoScreenDetected) {
      sessao.lastResultScreenDetected = false;
      sessao.visionAnalyzer.processarDeteccao(
        null,
        0,
        false,
        0,
    ...
    const tCropEnd = Date.now();
    const captureToCrop = tCropEnd - tFrameStart;
    ...
    const cropToRecognizer = tRecStart - tCropEnd;
    ...
    const tRecEnd = Date.now();
    const recognizerToMatcher = tRecEnd - tRecStart;
    ...
    logger.info(
      `[LOCAL_CAPTURE]\n` +
      `frameId=${(framePayload as any).frameId || sessao.totalFrames}\n` +
      ...
      `roiHeight=${roi.height}\n` +
      `roiFound=${roi.found}\n` +
      `resScreenDetected=${resScreenDetection.resultadoScreenDetected}\n` +
      `resScreenConfidence=${resScreenDetection.confidence}\n` +
      `symbolCropX=${resScreenDetection.roi?.symbolCropX}\n` +
      `symbolCropY=${resScreenDetection.roi?.symbolCropY}\n` +
      `symbolCropWidth=${resScreenDetection.roi?.symbolCropWidth}\n` +
      `symbolCropHeight=${resScreenDetection.roi?.symbolCropHeight}\n` +
      `cropBytes=${cropBytes}\n` +
      `objectAreaEstimate=${localRes.objectAreaEstimate || 0}\n` +
      `objectAreaPercentage=${localRes.objectAreaPercentage || 0}%`
    );

    logger.info(
      `[LOCAL_MATCH]\n` +
      `winner=${localRes.candidato1}\n` +
      `winnerScore=${Math.round(localRes.score1 * 100)}%\n` +
      `second=${localRes.candidato2}\n` +
      `secondScore=${Math.round(localRes.score2 * 100)}%\n` +
      `gap=${Math.round(localRes.gap * 100)}%\n` +
      `accepted=${localRes.accepted}\n` +
      `reason=${localRes.reason}\n` +
      `cacheHits=${localRes.cacheStats?.cacheHits ?? 0}\n` +
      `cacheMisses=${localRes.cacheStats?.cacheMisses ?? 0}`
    );
    ...
    if (!isResultScreenConfirmed) {
      ...
      const hasCandidate = localRes.objetoDetectado !== 'nenhum' && localRes.candidato1 !== 'nenhum' && isAllowedWheelObject(localRes.candidato1);
      ...
      if (hasCandidate) {
        ...
        resultScreenDiagnostico.localDecision = localRes.accepted
          ? 'ACCEPT'
          : (localRes.gap < 0.03 ? 'AMBIGUOUS' : 'WEAK_CANDIDATE');
      } else {
        candidateObject = null;
        candidateConfidence = 0;
        recognitionMethod = 'NENHUM';
      ...
      logger.info(
        `[LOCAL_DECISION]\n` +
        `decision=${resultScreenDiagnostico.localDecision}\n` +
    ...
    const tAnaStart = Date.now();
    const matcherToAnalyzer = tAnaStart - tRecEnd;
    ...
    const totalLocalMs = tAnaEnd - tFrameStart;

    logger.info(
      `[LOCAL_TIMING]\n` +
      `captureToCrop=${captureToCrop}ms\n` +
      `cropToRecognizer=${cropToRecognizer}ms\n` +
      `recognizerToMatcher=${recognizerToMatcher}ms\n` +
      `matcherToAnalyzer=${matcherToAnalyzer}ms\n` +
      `total=${totalLocalMs}ms`
    );
    ...
    if (localConfig.LOCAL_ONLY_MODE || recognitionMethod === 'LOCAL') {
      ...
      const localRecognizerResultObj = {
        object: localRes.objetoDetectado !== 'nenhum' && isAllowedWheelObject(localRes.candidato1) ? localRes.candidato1 : null,
        confidence: Math.round(localRes.score1 * 100),
        gap: Math.round(localRes.gap * 100),
        secondCandidate: localRes.candidato2 || null,
        status: (localRes.accepted && isAllowedWheelObject(localRes.candidato1) ? 'VALID' : (localRes.objetoDetectado === 'nenhum' ? 'NO_MATCH' : (localRes.gap < 0.03 ? 'AMBIGUOUS' : 'WEAK_CANDIDATE'))) as any,
      ...
      const uiStateObj: UIRecognitionState = {
        ...
        geminiConfidence: 0,
        localStatus: 'ATIVO',
        localObject: localRes.objetoDetectado !== 'nenhum' && isAllowedWheelObject(localRes.candidato1) ? localRes.candidato1 : null,
        localConfidence: Math.round(localRes.score1 * 100),
        localGap: Math.round(localRes.gap * 100),
        localDecision: localRes.accepted && isAllowedWheelObject(localRes.candidato1) ? 'ACCEPT' : (localRes.objetoDetectado === 'nenhum' ? 'REJECT' : (localRes.gap < 0.03 ? 'AMBIGUOUS' : 'WEAK_CANDIDATE')),
...

{
  ...
  private static userViewZoneConfig = {
    enabled: true,
    xPct: 10.0,
    yPct: 10.0,
    wPct: 80.0,
  ...
  public static getUserViewZoneConfig() {
    return { ...this.userViewZoneConfig };
  }

  public static updateUserViewZoneConfig(config: Partial<typeof BackendLiveService.userViewZoneConfig>) {
    const current = this.userViewZoneConfig;
    const xPct = Math.max(0, Math.min(95, config.xPct !== undefined ? Number(config.xPct) : current.xPct));
    const yPct = Math.max(0, Math.min(95, config.yPct !== undefined ? Number(config.yPct) : current.yPct));
    const maxW = 100 - xPct;
    const maxH = 100 - yPct;
    const wPct = Math.max(5, Math.min(maxW, config.wPct !== undefined ? Number(config.wPct) : current.wPct));
    const hPct = Math.max(5, Math.min(maxH, config.hPct !== undefined ? Number(config.hPct) : current.hPct));

    this.userViewZoneConfig = {
    ...
    logger.info(`[USER_VIEW_ZONE_UPDATED] X=${xPct}%, Y=${yPct}%, W=${wPct}%, H=${hPct}% (enabled=${this.userViewZoneConfig.enabled})`);
    return { ...this.userViewZoneConfig };
  }
}
