...
export class LocalWheelRecognizer {
  ...
  public static async recognizeCrop(
    ...
    try {
      ...
      const bestObj = visualMatch.simboloCandidatoVisual || cand1Obj;
      const isAccepted =
        bestObj !== 'nenhum' &&
      ...
      // Preserva o candidato se score >= 55% (0.55). Mínimo de visibilidade para fracos/provisórios.
      const objetoDetectado = score1 >= 0.55 ? bestObj : 'nenhum';
      const confianca = score1;
      ...
      const recognizerStatus = isAccepted
        ? 'CONFIRMATION_ELIGIBLE'
        : (objetoDetectado === 'nenhum'
            ? 'NO_CANDIDATE'
            : (gap < LocalWheelRecognizer.MIN_GAP_THRESHOLD ? 'AMBIGUOUS' : 'WEAK_CANDIDATE'));
      console.log(
        `[RECOGNIZER_OUTPUT]\n` +
      ...
      return {
        ...
        objectAreaEstimate: visualMatch.objectAreaEstimate,
        objectAreaPercentage: visualMatch.objectAreaPercentage,
        cacheStats: visualMatch.cacheStats || WheelObjectVisualMatcher.getReferenceCacheStats(),
        scoresBreakdown: topCand1 ? {
          centerScore: topCand1.centerScore,
          spatialScore: topCand1.spatialScore,
          hashScore: topCand1.hashScore,
          edgeScore: topCand1.edgeScore,
          histogramScore: topCand1.histogramScore,
        } : undefined,
      };
    } catch (error) {
