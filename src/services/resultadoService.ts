...
export const activeScreenLifecycleMap = new Map<string, ActiveScreenLifecycle>();
...
function normalizarObjetoLocal(item: string): string | null {
  if (!item) return null;
  const lower = item.toLowerCase().trim();
  if (lower.includes('sorvete')) return 'sorvete';
  if (lower.includes('boia') || lower.includes('bóia')) return 'boia';
  if (lower.includes('balao') || lower.includes('balão')) return 'balao';
  if (lower.includes('soco')) return 'soco';
  if (lower.includes('tedy') || lower.includes('teddy') || lower.includes('urso')) return 'tedy';
  if (lower.includes('princesa')) return 'princesa';
  if (lower.includes('camera') || lower.includes('câmera')) return 'camera';
  if (lower.includes('coroa')) return 'coroa';
  return null;
}
...
export async function registrarResultadoAutomaticamente(
  ...
  origemParam: string = 'gemini_live',
  multiplicadorParam?: number | string | null
): Promise<AutoRegisterResult> {
  const callId = `CALL_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
  const objetoFormatado = normalizarObjetoLocal(objeto) || (objeto || '').trim().toLowerCase();
  ...
  const LIFECYCLE_WINDOW_MS = 1000;
  const isLifecycleRecent = activeLifecycle && (Date.now() - activeLifecycle.timestamp < LIFECYCLE_WINDOW_MS);
  const isLiveSource = origemParam === 'gemini_live';
  ...
  if (
    ...
    activeLifecycle.objeto === objetoFormatado
  ) {
    logger.info(
  ...
  const executePersistence = async (): Promise<AutoRegisterResult> => {
    try {
      ...
      const payloadReal: any = {
        ...
        origem: origemParam || 'gemini_live',
        status: 'confirmado',
        criado_em: timestampIso,
        horario_resultado: timestampIso,
        ...(multiplicadorParam ? { observacao: JSON.stringify({ multiplicador: multiplicadorParam }) } : {}),
      };

      // VALIDAR PAYLOAD DO INSERT (REQUISITO #3)
      if (
        !payloadReal.objeto ||
        payloadReal.objeto === 'nenhum' ||
        payloadReal.objeto === 'nao_identificado' ||
        payloadReal.objeto === ''
      ) {
        logger.error(
          `[PERSIST_ERROR]\n` +
            `INVALID_PERSIST_PAYLOAD\n` +
            `campo: objeto\n` +
            `valor: ${payloadReal.objeto}`
        );
        activeScreenLifecycleMap.delete(sessionKey);
        return {
          registrado: false,
          motivo: `INVALID_PERSIST_PAYLOAD: campo 'objeto' inválido (${payloadReal.objeto})`,
      ...
      logger.info(`[PERSIST_TRACE] INSERT_PAYLOAD ${JSON.stringify(payloadReal)}`);
      ...
      if (insertError) {
        ...
        return {
          registrado: false,
          motivo: `PERSISTENCE_STATUS=FAILED: [${insertError.code}] ${insertError.message}`,
          sessaoId: numericSessaoId,
          rodadaRegistrada: null,
          eventId: eventIdEfetivo,
        };
      ...
      logger.info(`[PERSIST_TRACE] INSERT_SUCCESS id=${insertedData?.id}`);
      ...
      logger.info(`[PERSIST_TRACE] SELECT_VERIFY id=${insertedId}`);
      ...
      if (verifyData && verifyData.id === insertedId) {
        logger.info(`[PERSIST_TRACE] FOUND = true`);
        logger.info(
        ...
        logger.info(`[REGISTRO] sucesso (Objeto: "${objetoFormatado}" na rodada ${insertedRound || eventIdEfetivo})`);
        logger.info(`[ATUALIZAÇÃO] último registrado = ${ultimoObjetoRegistrado}`);
