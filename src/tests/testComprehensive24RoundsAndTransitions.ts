...
async function runComprehensiveDiagnosticTest() {
  ...
  for (const item of testPlan) {
    ...
    // Reset/Liberar para nova rodada limpa com transição de tela (3 frames false)
    analyzer.processarDeteccao(null, 0, false, 0, 'TEST_SESS_24', roundCounter, timestamp - 100, 0);
    analyzer.processarDeteccao(null, 0, false, 0, 'TEST_SESS_24', roundCounter, timestamp - 50, 0);
    analyzer.processarDeteccao(null, 0, false, 0, 'TEST_SESS_24', roundCounter, timestamp - 10, 0);

    // Frame 1, 2, 3: Processar detecções até obter confirmação
    let resAnalyzer = analyzer.processarDeteccao(item.object, 95, true, 0.95, 'TEST_SESS_24', roundCounter, timestamp, 12);
    if (!resAnalyzer.confirmedNow) {
      resAnalyzer = analyzer.processarDeteccao(item.object, 95, true, 0.95, 'TEST_SESS_24', roundCounter, timestamp + 50, 12);
    }
    if (!resAnalyzer.confirmedNow) {
      resAnalyzer = analyzer.processarDeteccao(item.object, 95, true, 0.95, 'TEST_SESS_24', roundCounter, timestamp + 100, 12);
    }
  ...
  for (const pair of transitionPairs) {
    ...
    const res1 = analyzer.processarDeteccao(pair.from, 92, true, 0.95, 'TEST_SESS_TRANS', 100, ts + 60, 10);
    ...
    // Simular ausência de tela por 3 frames (transição)
    ...
    analyzer.processarDeteccao(null, 0, false, 0, 'TEST_SESS_TRANS', 101, ts + 180, 0);
    ...
    const res2 = analyzer.processarDeteccao(pair.to, 94, true, 0.95, 'TEST_SESS_TRANS', 102, ts + 1660, 12);
