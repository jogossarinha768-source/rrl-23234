...
import { Camera, Database, HelpCircle, Sparkles, RefreshCw, BarChart3, ShieldCheck, Brain, Users, LogOut, Crop } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 text-center">
          ...
          <div className="flex items-center justify-center flex-wrap gap-2 pb-1 md:pb-0">
            ...
            {onOpenUserViewZone && (
              <button
                onClick={onOpenUserViewZone}
                className="px-3 py-2 bg-emerald-700 hover:bg-emerald-600 text-white rounded-lg text-xs font-extrabold flex items-center gap-1.5 shadow-md shadow-emerald-600/20 transition-all border border-emerald-400/30 whitespace-nowrap active:scale-95 cursor-pointer"
                title="Configurar a visão da roda permitida aos usuários"
              >
                <Crop className="w-4 h-4 text-emerald-200" />
                <span>Corte Usuários</span>
              </button>
            )}
            ...
            {onOpenUserMgmt && (
              <button
                ...
                className="px-3 py-2 bg-slate-800 hover:bg-cyan-500/20 text-cyan-300 border border-slate-700 hover:border-cyan-500/40 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all whitespace-nowrap cursor-pointer"
            ...
            {onLogout && (
              <button
                onClick={onLogout}
                className="p-2 bg-slate-800 hover:bg-rose-500/20 text-slate-400 hover:text-rose-300 rounded-lg text-xs font-semibold flex items-center justify-center border border-slate-700 hover:border-rose-500/40 transition-all cursor-pointer"
