...
import { UserViewZoneService, UserViewZoneConfig } from '../services/userViewZoneService';
...
export const LiveDevMetricsPanel: React.FC<LiveDevMetricsPanelProps> = ({
  ...
  // --- USER_VIEW_ZONE (Corte dos Usuários) ---
  const [isUserViewModalOpen, setIsUserViewModalOpen] = useState(false);
  const [userViewZoneCfg, setUserViewZoneCfg] = useState<UserViewZoneConfig>(() => {
    return UserViewZoneService.getUserViewZoneConfig();
  });
  const [userViewSaveToast, setUserViewSaveToast] = useState<string | null>(null);

  // Sincronizar com backend ao montar
  useEffect(() => {
    fetch('/api/live/user-view-zone')
      .then((res) => res.json())
      .then((data) => {
        if (data.sucesso && data.config) {
          setUserViewZoneCfg(data.config);
  ...
  const handleUserViewSliderChange = (field: keyof UserViewZoneConfig, val: number) => {
    const updated = {
      ...userViewZoneCfg,
      enabled: true,
      [field]: val,
    };
    setUserViewZoneCfg(updated);
    UserViewZoneService.setUserViewZoneConfig(updated);
  };

  const handleSaveUserViewConfig = async () => {
    const updated = UserViewZoneService.setUserViewZoneConfig({
      ...userViewZoneCfg,
      enabled: true,
    });
    try {
    ...
    setUserViewSaveToast('Área dos Usuários (USER_VIEW_ZONE) salva com sucesso!');
    setTimeout(() => setUserViewSaveToast(null), 3000);
  };

  const handleResetUserViewConfig = async () => {
    const defaultConfig = UserViewZoneService.resetUserViewZoneConfig();
    setUserViewZoneCfg(defaultConfig);
    try {
      await fetch('/api/live/user-view-zone', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    setUserViewSaveToast('USER_VIEW_ZONE restaurada para a área padrão.');
    setTimeout(() => setUserViewSaveToast(null), 3000);
  };
  ...
  return (
    <div className="p-3.5 bg-slate-950/95 border-t border-indigo-500/30 rounded-b-3xl font-sans text-xs space-y-3">
          ...
          <div className="bg-slate-900/95 border border-cyan-500/50 p-3.5 rounded-2xl space-y-2.5 font-mono">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-[10px]">
              <div className="p-2 bg-slate-950 rounded-xl border border-slate-800 space-y-0.5">
                <div className="text-slate-500 text-[8px] uppercase font-bold">1. Captura → Recognizer</div>
                <div className="text-cyan-300 font-black text-xs">{lastResult?.latencia?.latenciaCapturaParaDeteccaoMs ?? captureToGemini} ms</div>
              </div>
              <div className="p-2 bg-slate-950 rounded-xl border border-slate-800 space-y-0.5">
                <div className="text-slate-500 text-[8px] uppercase font-bold">2. Recognizer → Analyzer</div>
                <div className="text-indigo-300 font-black text-xs">&lt; 1 ms</div>
              </div>
              <div className="p-2 bg-slate-950 rounded-xl border border-slate-800 space-y-0.5">
                <div className="text-slate-500 text-[8px] uppercase font-bold">3. Tempo Total Confirmação</div>
                <div className="text-emerald-300 font-black text-xs">{lastResult?.latencia?.latenciaTotalMs ?? totalTime} ms</div>
              </div>
              <div className="p-2 bg-slate-950 rounded-xl border border-slate-800 space-y-0.5">
                <div className="text-slate-500 text-[8px] uppercase font-bold">4. Frames Processados</div>
                <div className="text-amber-300 font-black text-xs">{framesEnviados}</div>
              </div>
              <div className="p-2 bg-slate-950 rounded-xl border border-slate-800 space-y-0.5">
                <div className="text-slate-500 text-[8px] uppercase font-bold">5. Status Gemini</div>
                <div className="text-slate-400 font-black text-xs">DESABILITADO</div>
              </div>
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {/* PAINEL UNIFICADO: DISTINÇÃO EXPLÍCITA ENTRE GATE DE TELA E GATE DE OBJETO */}
                <div className={`p-2.5 rounded-xl border text-[10px] font-mono flex flex-wrap items-center justify-between gap-2 ${
                  recAllowed && decision === 'ACCEPT'
                    ? 'bg-emerald-950/70 border-emerald-500/80 text-emerald-200'
                    : recAllowed && decision === 'AMBIGUOUS'
                    ? 'bg-amber-950/70 border-amber-500/80 text-amber-200'
                    : 'bg-slate-900 border-slate-800 text-slate-300'
                }`}>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-extrabold uppercase text-[9px] text-purple-300 bg-purple-950/80 px-2 py-0.5 rounded border border-purple-500/40">
                      PIPELINE STATUS
                    </span>
                    <span>
                      1. Gate da Tela: <strong className={recAllowed ? 'text-emerald-400 font-bold' : 'text-slate-400 font-bold'}>
                        {recAllowed ? 'CONFIRMADO (2/2) ✅' : 'FECHADO/AUSENTE ⏳'}
                      </strong>
                    </span>
                    <span className="text-slate-600">|</span>
                    <span>
                      2. Objeto (Símbolo): <strong className={
                        decision === 'ACCEPT'
                        ...
                        {decision === 'ACCEPT'
                          ? `CONFIRMADO (${winner}) ✅`
                          : decision === 'AMBIGUOUS'
                          ? `AMBÍGUO (Maior score: ${winner} ${winnerScore}%, Gap: ${gapPct}%) ⏳`
                          : 'NÃO IDENTIFICADO ❌'}
                      </strong>
                    </span>
                  </div>
                  <div className="text-[9px] font-bold text-slate-400">
                    {recAllowed && decision === 'AMBIGUOUS' && (
                      <span className="text-amber-300">
                        ⚠️ Tela OK, mas objeto necessita conf ≥ 85% e gap ≥ 3.0% (Aguardando frame limpo)
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="p-3 bg-slate-950 rounded-xl border border-indigo-500/40 space-y-2.5">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 text-[9px] text-slate-400 bg-slate-900 p-2 rounded-lg border border-slate-800">
                    <div>
                      1. GATE DA TELA: <strong className={recAllowed ? 'text-emerald-300 font-extrabold' : 'text-amber-300 font-bold'}>{recAllowed ? 'CONFIRMADO (2/2) ✅' : 'FECHADO/AUSENTE ⏳'}</strong>
                    </div>
                    <div>
                      2. OBJETO: <strong className={decision === 'ACCEPT' ? 'text-emerald-300 font-extrabold' : 'text-amber-300 font-bold'}>
                        {decision === 'ACCEPT' ? 'CONFIRMADO ✅' : (decision === 'AMBIGUOUS' ? 'AMBÍGUO ⏳' : 'NÃO IDENTIFICADO ❌')}
                      </strong>
                    </div>
                    <div>
                      WINNER: <strong className="text-emerald-300 capitalize font-bold">{winner} ({winnerScore}%)</strong>
                    </div>
                    <div>
                      2º COLOCADO: <strong className="text-blue-300 capitalize font-bold">{second} ({Math.round(winnerScore - gapPct)}%)</strong>
                    </div>
                    <div>
                      GAP: <strong className="text-cyan-300 font-bold">{gapPct}%</strong>
                    </div>
                  </div>
                </div>
              </>
      ...
      {(() => {
        ...
        const localScores = resDiag?.localScoresPorObjeto || {};
        const localWinScore = Math.round((resDiag?.localConfidence || 0) * 100);
        const localDec = resDiag?.localDecision || 'REJECT';

        const isLocalOnly =
          lastResult?.geminiResult?.status === 'DISABLED' ||
          lastResult?.rawText?.startsWith('LOCAL_ONLY_MODE') ||
          lastTrace?.geminiTag === 'LOCAL_ONLY_MODE' ||
          resDiag?.geminiFallbackReason === 'GEMINI_DISABLED_LOCAL_ONLY';
        ...
        const stepGeminiResp = isLocalOnly
          ? 'SKIPPED'
          : isGeminiBlocked
          ? 'BLOCKED'
          : !!resDiag?.geminiRawResponse ||
            (!!lastResult?.rawText && lastResult.rawText.length > 0 && !lastResult.rawText.startsWith('LOCAL_ONLY_MODE')) ||
            (!!lastTrace?.geminiRaw && lastTrace.geminiRaw.length > 0 && !lastTrace.geminiRaw.includes('GEMINI_TIMEOUT') && !lastTrace.geminiRaw.includes('LOCAL_ONLY_MODE')) ||
            (!!status.ultimaRespostaBrutaGemini && status.ultimaRespostaBrutaGemini !== '(sem resposta do modelo)' && !status.ultimaRespostaBrutaGemini.includes('GEMINI_TIMEOUT') && !status.ultimaRespostaBrutaGemini.includes('LOCAL_ONLY_MODE'));

        const stepParser = isLocalOnly || isGeminiBlocked
          ? 'SKIPPED'
        ...
        const stepMatcher = isLocalOnly
          ? (localWinScore > 0 || Object.keys(localScores).length > 0 || resDiag?.matcherInput !== undefined)
          : isGeminiBlocked
          ? 'SKIPPED'
        ...
        const stepFinal = isLocalOnly
          ? (localDec === 'ACCEPT' ? true : (localDec === 'AMBIGUOUS' || localDec === 'REJECT' ? 'BLOCKED' : false))
          : isGeminiBlocked
          ? 'SKIPPED'
        ...
        return (
          <div className="bg-slate-900/95 border border-purple-500/40 p-3.5 rounded-2xl space-y-3 font-mono">
            ...
            <div className="p-3 bg-slate-950 rounded-xl border border-cyan-500/50 space-y-3">
              ...
              {manualCropResult && (
                <div className="p-3 bg-black rounded-lg border border-cyan-400/60 space-y-3">
                  <div className="flex items-center justify-between text-[10px] border-b border-slate-800 pb-1">
                    <span className="font-black text-cyan-300 uppercase flex items-center gap-1.5">
                      ...
                      🧪 ANÁLISE DO CROP REAL (RECONHECIMENTO VISUAL LOCAL)
                    </span>
                    <div className="flex items-center gap-2">
                      <span className="text-slate-400 font-mono text-[9px]">Crop: {manualCropResult.cropSize || '153x153'}</span>
                      <span className="text-emerald-400 font-bold font-mono text-[9px]">{manualCropResult.latencyMs || 0}ms</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-6 gap-2 text-[10px] font-mono">
                    <div className="p-2 bg-slate-900 rounded border border-amber-500/30">
                      <span className="text-slate-400 uppercase font-bold text-[8px] block">WINNER</span>
                      <div className="text-amber-300 font-black text-xs capitalize">
                        {manualCropResult.winner || manualCropResult.objetoDetectado || manualCropResult.finalObject || 'nenhum'}
                      </div>
                      <span className="text-amber-200/80 text-[9px] font-bold">
                        {manualCropResult.winnerScore ?? manualCropResult.confianca ?? 0}%
                      </span>
                    </div>

                    <div className="p-2 bg-slate-900 rounded border border-slate-800">
                      <span className="text-slate-400 uppercase font-bold text-[8px] block">SECOND</span>
                      <div className="text-slate-300 font-bold text-xs capitalize">
                        {manualCropResult.second || manualCropResult.visualMatch?.objeto || 'nenhum'}
                      </div>
                      <span className="text-slate-400 text-[9px] font-bold">
                        {manualCropResult.secondScore ?? manualCropResult.visualMatch?.confianca ?? 0}%
                      </span>
                    </div>

                    <div className="p-2 bg-slate-900 rounded border border-slate-800">
                      <span className="text-slate-400 uppercase font-bold text-[8px] block">GAP</span>
                      <div className="text-cyan-300 font-black text-xs">
                        {manualCropResult.gap ?? 0}%
                      </div>
                    </div>

                    <div className="p-2 bg-slate-900 rounded border border-slate-800">
                      <span className="text-slate-400 uppercase font-bold text-[8px] block">ACCEPTED</span>
                      <div className={`font-black text-xs ${manualCropResult.accepted ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {manualCropResult.accepted ? 'SIM' : 'NÃO'}
                      </div>
                    </div>

                    <div className="col-span-2 p-2 bg-slate-900 rounded border border-slate-800">
                      <span className="text-slate-400 uppercase font-bold text-[8px] block">REASON</span>
                      <div className="text-cyan-300 font-mono text-[9px] truncate" title={manualCropResult.reason || 'VALID_MATCH'}>
                        {manualCropResult.reason || manualCropResult.error || 'VALID_MATCH'}
                      </div>
                    </div>
                  </div>

                  {/* TABELA DE PONTUAÇÃO DOS 8 OBJETOS (ORDENADOS DO MAIOR PARA O MENOR) */}
                  {manualCropResult.scoresPorObjeto && (
                    <div className="pt-2 border-t border-slate-800">
                      <div className="text-[9px] font-bold text-slate-300 uppercase mb-1 flex justify-between items-center">
                        <span>SCORES DOS 8 OBJETOS (MATCHER LOCAL):</span>
                        <span className="text-[8px] text-cyan-400">100% LOCAL ONLY (SEM GEMINI)</span>
                      </div>
                      <div className="grid grid-cols-4 sm:grid-cols-8 gap-1.5 text-center font-mono text-[9px]">
                        {Object.entries(manualCropResult.scoresPorObjeto)
                          .map(([objName, scoreVal]: [string, any]) => ({
                            objName,
                            scorePct: Math.round((Number(scoreVal) || 0) * (Number(scoreVal) > 1 ? 1 : 100)),
                          }))
                          .sort((a, b) => b.scorePct - a.scorePct)
                          .map(({ objName, scorePct }) => {
                            const isWinner = objName === (manualCropResult.winner || manualCropResult.objetoDetectado);
                            const isSecond = objName === manualCropResult.second;
                            return (
                              <div
                                key={objName}
                                className={`p-1.5 rounded border flex flex-col items-center justify-between ${
                                  isWinner
                              ...
                              >
                                <span className={`font-bold capitalize text-[8px] ${isWinner ? 'text-amber-300' : 'text-slate-400'}`}>
                                  {objName}
                                </span>
                                <span className={`font-black text-xs ${isWinner ? 'text-amber-300' : isSecond ? 'text-cyan-300' : 'text-slate-300'}`}>
                                  {scorePct.toFixed(1)}%
                                </span>
                              </div>
                            );
            ...
            <div className="mt-3 p-3 bg-slate-950/90 border border-purple-500/40 rounded-xl space-y-2.5 font-mono">
              ...
              {/* SEÇÃO DA USER_VIEW_ZONE (CORTE DOS USUÁRIOS) */}
              <div className="border-t border-slate-800/80 pt-3 mt-3 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-extrabold text-emerald-200 uppercase tracking-wider">
                      CONFIGURAÇÃO DA VISUALIZAÇÃO DOS USUÁRIOS (USER_VIEW_ZONE)
                    </span>
                    <span className="text-[8px] bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 px-1.5 py-0.5 rounded font-bold">
                      CORTE INDEPENDENTE
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setIsUserViewModalOpen(true)}
                      className="px-2.5 py-1 bg-emerald-700 hover:bg-emerald-600 text-white text-[10px] font-bold rounded flex items-center gap-1 shadow transition-all cursor-pointer"
                    >
                      <Crop className="w-3 h-3" /> AJUSTAR CORTE GRAFICAMENTE
                    </button>
                    <button
                      onClick={handleSaveUserViewConfig}
                      className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-bold rounded flex items-center gap-1 shadow transition-all cursor-pointer"
                    >
                      <Save className="w-3 h-3" /> SALVAR ÁREA DOS USUÁRIOS
                    </button>
                    <button
                      onClick={handleResetUserViewConfig}
                      className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] font-medium rounded flex items-center gap-1 transition-all cursor-pointer"
                    >
                ...
                {userViewSaveToast && (
                  <div className="text-[9px] bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 p-1.5 rounded flex items-center gap-1.5 font-bold">
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                    <span>{userViewSaveToast}</span>
                  </div>
                )}

                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-0.5 text-[10px]">
                  <div className="space-y-1 bg-slate-900/60 p-2 rounded border border-emerald-500/20">
                    <div className="flex justify-between text-slate-300">
                      <span>Posição X (%):</span>
                      <strong className="text-emerald-400">{userViewZoneCfg.xPct.toFixed(1)}%</strong>
                    </div>
                    <input
                      type="range"
                      min="0"
                      max="90"
                      step="0.5"
                      value={userViewZoneCfg.xPct}
                      onChange={(e) => handleUserViewSliderChange('xPct', parseFloat(e.target.value))}
                  ...
                  <div className="space-y-1 bg-slate-900/60 p-2 rounded border border-emerald-500/20">
                    <div className="flex justify-between text-slate-300">
                      <span>Posição Y (%):</span>
                      <strong className="text-emerald-400">{userViewZoneCfg.yPct.toFixed(1)}%</strong>
                    </div>
                    <input
                      type="range"
                      min="0"
                      max="90"
                      step="0.5"
                      value={userViewZoneCfg.yPct}
                      onChange={(e) => handleUserViewSliderChange('yPct', parseFloat(e.target.value))}
                      className="w-full accent-emerald-500 bg-slate-800 h-1.5 rounded cursor-pointer"
                    />
                  </div>
                  ...
                  <div className="space-y-1 bg-slate-900/60 p-2 rounded border border-emerald-500/20">
                    <div className="flex justify-between text-slate-300">
                      <span>Largura W (%):</span>
                      <strong className="text-amber-300">{userViewZoneCfg.wPct.toFixed(1)}%</strong>
                    </div>
                    <input
                      type="range"
                      min="10"
                      max={100 - userViewZoneCfg.xPct}
                      step="0.5"
                      value={userViewZoneCfg.wPct}
                      onChange={(e) => handleUserViewSliderChange('wPct', parseFloat(e.target.value))}
                      className="w-full accent-emerald-500 bg-slate-800 h-1.5 rounded cursor-pointer"
                    />
                  </div>
                  ...
                  <div className="space-y-1 bg-slate-900/60 p-2 rounded border border-emerald-500/20">
                    <div className="flex justify-between text-slate-300">
                      <span>Altura H (%):</span>
                      <strong className="text-amber-300">{userViewZoneCfg.hPct.toFixed(1)}%</strong>
                    </div>
                    <input
                      type="range"
                      min="10"
                      max={100 - userViewZoneCfg.yPct}
                      step="0.5"
                      value={userViewZoneCfg.hPct}
                      onChange={(e) => handleUserViewSliderChange('hPct', parseFloat(e.target.value))}
                      className="w-full accent-emerald-500 bg-slate-800 h-1.5 rounded cursor-pointer"
                    />
                  </div>
                </div>
              ...
              {/* Modal Interativo da User View Zone */}
              <UserViewZoneModal
                isOpen={isUserViewModalOpen}
                onClose={() => setIsUserViewModalOpen(false)}
                currentFrameUrl={lastCapturedFrameDataUrl}
              />
            </div>
          </div>
        );
