...
/**
 * GET /api/live/user-view-zone
 * Retorna a configuração oficial do Corte dos Usuários (USER_VIEW_ZONE).
 */
router.get('/live/user-view-zone', (req, res) => {
  try {
    const config = BackendLiveService.getUserViewZoneConfig();
    return res.json({
      sucesso: true,
      config,
    });
  } catch (error: any) {
    logger.error('Erro na rota GET /api/live/user-view-zone:', error?.message);
    return res.status(500).json({
      sucesso: false,
      error: error?.message || 'Erro ao consultar configuração da USER_VIEW_ZONE.',
    });
  }
});

/**
 * POST /api/live/user-view-zone
 * Atualiza a área do Corte dos Usuários (USER_VIEW_ZONE) definida pelo Administrador.
 */
router.post('/live/user-view-zone', (req, res) => {
  try {
    const newConfig = req.body || {};
    const updated = BackendLiveService.updateUserViewZoneConfig(newConfig);
    return res.json({
      sucesso: true,
      config: updated,
    });
  } catch (error: any) {
    logger.error('Erro na rota POST /api/live/user-view-zone:', error?.message);
    return res.status(500).json({
      sucesso: false,
      error: error?.message || 'Erro ao atualizar área da USER_VIEW_ZONE.',
    });
  }
});
